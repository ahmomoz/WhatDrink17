import"./bootstrap.min-feac71ab.js";import{b as $,c as y,e as x,d as w,a as C}from"./api-4c8ad0b6.js";import{A as p}from"./config-ca05e4f1.js";document.addEventListener("DOMContentLoaded",()=>{const r=document.querySelector("#lastestNews");let c=[];function f(){let i="";c.forEach(function(d){i+=`
...
`}),r.innerHTML=n},v=()=>{k()};function S(){document.querySelectorAll(".collect-drink-btn").forEach(function(t){t.addEventListener("click",function(s){const o=this.getAttribute("data-drink-id");s.target.value=="collected"?(t.value="uncollect",t.classList.add("fa-regular"),t.classList.remove("fa-solid"),y(L,o),Swal.fire("已取消收藏")):s.target.value=="uncollect"&&(t.value="collected",t.classList.remove("fa-regular"),t.classList.add("fa-solid"),x(L,o),Swal.fire("收藏成功"))})})}});document.addEventListener("DOMContentLoaded",()=>{const r=document.querySelector("#popularStoreList");let c=[],f=[],i=[],d=[];const u=sessionStorage.getItem("user_id");function b(){window.location.href="logIn.html"}Promise.all([axios.get("https://json-server-project-wtkt.onrender.com/userShopCollections"),axios.get("https://json-server-project-wtkt.onrender.com/shops")]).then(t=>{i=t[0].data,c=t[1].data,k(),m(),S()}).catch(t=>{console.error("Error fetching data:",t)});const m=()=>{c.forEach(t=>{const s={uber:"uber eat",foodpanda:"food panda",hasEvent:"合作活動"};let o="";for(const a in s)t[a]&&(o+=`<li class="stores-tag">${s[a]}</li>`);f.push(o),o=""})};function k(){let t={};for(var s=0;s<i.length;s++){let l=i[s].shopId;t[l]?t[l]++:t[l]=1}const o=Object.entries(t);o.sort(function(l,h){return h[1]-l[1]});let e=o.slice(0,4).map(function(l){return l[0]});c.forEach(function(l){e.forEach(function(h){h==l.id&&d.push(l)})})}const v=()=>{let t="",s=[];axios.get(`https://drinkpicker-nclv.onrender.com/userShopCollections?userId=${u}`).then(function(o){let a=[];a=o.data;for(let e=0;e<a.length;e++)s[e]=a[e].shopId;s=s.map(e=>Number(e)).filter(e=>!isNaN(e)),d.forEach(e=>{const l=s.includes(e.id)?`<button type="button" class="collect-btn collect-store-btn border-0 text-primary fa-heart fs-24 fa-solid" data-shop-id="${e.id}" value="collected" aria-hidden="true"></button>`:`<button type="button" class="collect-btn collect-store-btn border-0 text-primary fa-regular fa-heart fs-24" data-shop-id="${e.id}" value="uncollect" aria-hidden="true"></button>`;t+=`
        ...
        `}),r.innerHTML=t})},S=()=>{v(),n()},n=()=>{};r.addEventListener("click",function(t){if(t.target.closest(".collect-btn")){if(!u)b();else if(t.target.classList.contains("collect-btn")){const o=t.target,e=o.closest(".stores-card").dataset.shopId,l=`${p}/userShopCollections?userId=${u}&shopId=${e}`;axios.get(l).then(h=>{if(h.data.length>0){w(u,e),o.value="uncollect",o.classList.remove("fa-solid"),o.classList.add("fa-regular"),Swal.fire("已取消收藏");return}else C(u,e),o.value="collected",o.classList.remove("fa-regular"),o.classList.add("fa-solid"),Swal.fire("收藏成功")}).catch(h=>{console.error("Error checking collection status:",h)})}}})});const E=document.querySelector("#searchButton"),g=document.querySelector("#searchInput"),D=document.querySelectorAll(".tagSearch");E.addEventListener("click",function(){I()});g.addEventListener("keydown",r=>{r.keyCode===13&&I()});D.forEach(r=>{r.addEventListener("click",function(){let c=r.getAttribute("value");T(c)})});function I(){let r=g.value;localStorage.setItem("searchTerm",r),window.location.href="searchResult.html",g.value=""}function T(r){let c=r;localStorage.setItem("searchTerm",c),window.location.href="searchResult.html",g.value=""}const A=document.querySelectorAll(".collect-btn");function N(){A.forEach(function(r){r.addEventListener("click",function(c){c.target.value=="collected"?(r.value="uncollect",r.classList.remove("fa-regular"),r.classList.add("fa-solid")):c.target.value=="uncollect"&&(r.value="collected",r.classList.add("fa-regular"),r.classList.remove("fa-solid")),console.log("hi")})})}N();
