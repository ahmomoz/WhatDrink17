import"./bootstrap.min-feac71ab.js";import{c as j,e as q}from"./api-4c8ad0b6.js";import{A as S}from"./config-ca05e4f1.js";const p=document.querySelector("#searchDrinkList"),$=document.querySelector("#searchBreadcrumb");let y=document.querySelector("#teaTypeSelect"),x=document.querySelector("#ingredientsSelect"),w=[],o=[],h=[];const u=sessionStorage.getItem("user_id"),A=()=>{const t=o.map(a=>!a||!a.TeaType?"":a.Ingredients.length===0?`${a.TeaType}`:`${a.TeaType},${a.Ingredients}`),e=[];for(let a=0;a<t.length;a++)e.push(t[a].split(","));let n="",s=[];e.forEach(a=>{a.forEach((l,f)=>{n+=`<li class="drinks-tag">${l}</li>`,f===a.length-1&&(s.push(n),n="")}),h.push(s),s=[]})},M=()=>{let t="";o.forEach(e=>{t+=`
