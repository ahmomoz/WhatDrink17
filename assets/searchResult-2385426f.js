import"./bootstrap.min-feac71ab.js";const p=document.querySelector("#searchDrinkList"),$=document.querySelector("#searchBreadcrumb");let y=document.querySelector("#teaTypeSelect"),x=document.querySelector("#ingredientsSelect"),T=[],m=[],b=[];const h=sessionStorage.getItem("user_id"),_=()=>{const t=m.map(a=>!a||!a.TeaType?"":a.Ingredients.length===0?`${a.TeaType}`:`${a.TeaType},${a.Ingredients}`),e=[];for(let a=0;a<t.length;a++)e.push(t[a].split(","));let n="",s=[];e.forEach(a=>{a.forEach((g,c)=>{n+=`<li class="drinks-tag">${g}</li>`,c===a.length-1&&(s.push(n),n="")}),b.push(s),s=[]})},N=()=>{let t="";m.forEach(e=>{t+=`
