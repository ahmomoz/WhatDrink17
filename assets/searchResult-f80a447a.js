import"./bootstrap.min-66c8f441.js";const p=document.querySelector("#searchDrinkList"),$=document.querySelector("#searchBreadcrumb");let y=document.querySelector("#teaTypeSelect"),x=document.querySelector("#ingredientsSelect"),w=[],m=[],b=[];const h=sessionStorage.getItem("user_id"),_=()=>{const t=m.map(a=>!a||!a.TeaType?"":a.Ingredients.length===0?`${a.TeaType}`:`${a.TeaType},${a.Ingredients}`),e=[];for(let a=0;a<t.length;a++)e.push(t[a].split(","));let n="",s=[];e.forEach(a=>{a.forEach((g,c)=>{n+=`<li class="drinks-tag">${g}</li>`,c===a.length-1&&(s.push(n),n="")}),b.push(s),s=[]})},N=()=>{let t="";m.forEach(e=>{t+=`
    ...
    `}),p.innerHTML=e}const v=t=>{const e=t.length,n=10,s=Math.ceil(e/n);let a=1;function g(o){const d=(o-1)*n,i=d+n,f=t.slice(d,i);c(f),L(),k()}function c(o){p.innerHTML="";let d="";o.forEach(i=>{d+=`
    ...
    `;const i=a===1?"text-gray":"",f=a===s?"text-gray":"",P=a===1?"disabled":"",S=a===1?"disabled":"",B=a===s?"disabled":"",C=a===s?"disabled":"";o.innerHTML=`
    ...
    </li>`,document.querySelectorAll(".page-link").forEach(r=>{r.addEventListener("click",T=>{T.preventDefault();const j=parseInt(r.dataset.page);l(j)})});const u=[a];document.getElementById("previousPageBtn").addEventListener("click",r=>{r.preventDefault(),u[0]>1&&l(u[0]-1)}),document.getElementById("next-page-btn").addEventListener("click",r=>{r.preventDefault(),u[0]!==s&&l(u[0]+1)}),document.getElementById("first-page-btn").addEventListener("click",r=>{r.preventDefault(),u[0]>1&&l(1)}),document.getElementById("last-page-btn").addEventListener("click",r=>{r.preventDefault(),u[0]!==s&&l(s)})}function l(o){a=o,g(a),window.scrollTo(0,400)}g(a)},E=t=>{const e=document.querySelector("#searchDrinks"),n=()=>{let s=t.filter(a=>a.DrinkName.includes(e.value));I(s),v(s),$.textContent="搜尋結果",k()};e.addEventListener("keyup",s=>{s.keyCode===13&&n()})};function H(t){let e=localStorage.getItem("searchTerm");if(e){$.textContent=e;let n=t.filter(s=>s.DrinkName.includes(e));I(n),v(n)}}Promise.all([axios.get(`https://json-server-project-wtkt.onrender.com/userDrinkCollections?userId=${h}`),axios.get("https://json-server-project-wtkt.onrender.com/drinks")]).then(t=>{w=t[0].data,m=t[1].data,_(),q(),D(),v(m),H(m),k()}).catch(t=>{console.error("Error fetching data:",t)});
