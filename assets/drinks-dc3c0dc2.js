import"./bootstrap.min-feac71ab.js";const u=document.querySelector("#drinkList");let v=document.querySelector("#teaTypeSelect"),y=document.querySelector("#ingredientsSelect"),L=[],p=[],k=[];const b=sessionStorage.getItem("user_id"),C=()=>{const a=p.map(t=>!t||!t.TeaType?"":t.Ingredients.length===0?`${t.TeaType}`:`${t.TeaType},${t.Ingredients}`),e=[];for(let t=0;t<a.length;t++)e.push(a[t].split(","));let n="",s=[];e.forEach(t=>{t.forEach((m,d)=>{n+=`<li class="drinks-tag">${m}</li>`,d===t.length-1&&(s.push(n),n="")}),k.push(s),s=[]})},_=()=>{let a="";p.forEach(e=>{a+=`
        ...
        `}),u.innerHTML=a},N=()=>{_(),E(p)},h=()=>{const a=e=>document.querySelector(`.drinks-card[data-drink-id="${e}"] .collect-btn`);L.forEach(e=>{const n=a(e.drinkId);n&&(n.value="collected",n.classList.remove("fa-regular"),n.classList.add("fa-solid"))})};u.addEventListener("click",function(a){const e=a.target.closest(".collect-btn");if(e){if(!b)console.log("無權限: 沒有找到 Token"),M();else if(a.target.classList.contains("collect-btn")){const s=e.closest(".drinks-card").dataset.drinkId;console.log(s);const t="https://json-server-project-wtkt.onrender.com/userDrinkCollections",m=`${t}?userId=${b}&drinkId=${s}`;axios.get(m).then(d=>{if(d.data.length>0){e.value="uncollect",e.classList.remove("fa-solid"),e.classList.add("fa-regular"),axios.delete(`${t}/${d.data[0].id}`).then(r=>{Swal.fire("已取消收藏")}).catch(r=>{console.error("Error deleting collection:",r)});return}else e.value="collected",e.classList.remove("fa-regular"),e.classList.add("fa-solid");axios.post(t,{userId:parseInt(b),drinkId:parseInt(s)}).then(r=>{Swal.fire("收藏成功")}).catch(r=>{console.error("Error adding collection:",r)})}).catch(d=>{console.error("Error checking collection status:",d)})}}});function M(){alert("登入後即可使用收藏功能"),window.location.href="logIn.html"}v.addEventListener("change",x);y.addEventListener("change",x);function x(){let a=p;v.value!==""&&(a=a.filter(e=>e.TeaType.includes(v.value))),y.value!==""&&(a=a.filter(e=>e.Ingredients.includes(y.value))),w(a),E(a),$(a),h()}function w(a){let e="";a.forEach(n=>{e+=`
    ...
    `}),u.innerHTML=e}const $=a=>{const e=a.length,n=10,s=Math.ceil(e/n);let t=1;function m(c){const o=(c-1)*n,i=o+n,f=a.slice(o,i);d(f),I(),h()}function d(c){u.innerHTML="";let o="";c.forEach(i=>{o+=`
        ...
        `}),u.innerHTML=o}function I(){const c=document.getElementById("pagination");c.innerHTML="";let o="";for(let l=1;l<=s;l++)o+=`
