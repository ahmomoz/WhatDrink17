import"./bootstrap.min-faf78b7c.js";const u=document.querySelector("#drinkList");let v=document.querySelector("#teaTypeSelect"),y=document.querySelector("#ingredientsSelect"),L=[],p=[],k=[];const b=sessionStorage.getItem("user_id"),C=()=>{const a=p.map(t=>!t||!t.TeaType?"":t.Ingredients.length===0?`${t.TeaType}`:`${t.TeaType},${t.Ingredients}`),e=[];for(let t=0;t<a.length;t++)e.push(a[t].split(","));let n="",s=[];e.forEach(t=>{t.forEach((m,d)=>{n+=`<li class="drinks-tag">${m}</li>`,d===t.length-1&&(s.push(n),n="")}),k.push(s),s=[]})},_=()=>{let a="";p.forEach(e=>{a+=`
