import"./bootstrap.min-feac71ab.js";import{c as j,e as q}from"./api-4c8ad0b6.js";import{A as S}from"./config-ca05e4f1.js";const p=document.querySelector("#searchDrinkList"),$=document.querySelector("#searchBreadcrumb");let y=document.querySelector("#teaTypeSelect"),x=document.querySelector("#ingredientsSelect"),T=[],o=[],h=[];const u=sessionStorage.getItem("user_id"),A=()=>{const t=o.map(a=>!a||!a.TeaType?"":a.Ingredients.length===0?`${a.TeaType}`:`${a.TeaType},${a.Ingredients}`),e=[];for(let a=0;a<t.length;a++)e.push(t[a].split(","));let n="",s=[];e.forEach(a=>{a.forEach((l,f)=>{n+=`<li class="drinks-tag">${l}</li>`,f===a.length-1&&(s.push(n),n="")}),h.push(s),s=[]})},M=()=>{let t="";o.forEach(e=>{t+=`
    ...
    </li>`,document.querySelectorAll(".page-link").forEach(i=>{i.addEventListener("click",L=>{L.preventDefault();const N=parseInt(i.dataset.page);g(N)})});const m=[a];document.getElementById("previousPageBtn").addEventListener("click",i=>{i.preventDefault(),m[0]>1&&g(m[0]-1)}),document.getElementById("next-page-btn").addEventListener("click",i=>{i.preventDefault(),m[0]!==s&&g(m[0]+1)}),document.getElementById("first-page-btn").addEventListener("click",i=>{i.preventDefault(),m[0]>1&&g(1)}),document.getElementById("last-page-btn").addEventListener("click",i=>{i.preventDefault(),m[0]!==s&&g(s)})}function g(c){a=c,l(a),window.scrollTo(0,400)}l(a)},w=t=>{const e=document.querySelector("#searchDrinks"),n=()=>{let s=t.filter(a=>a.DrinkName.includes(e.value));I(s),v(s),$.textContent="搜尋結果",k()};e.addEventListener("keyup",s=>{s.keyCode===13&&n()})};function U(t){let e=localStorage.getItem("searchTerm");if(e){$.textContent=e;let n=t.filter(s=>s.DrinkName.includes(e));I(n),v(n)}}Promise.all([axios.get(`${S}/userDrinkCollections?userId=${u}`),axios.get("https://json-server-project-wtkt.onrender.com/drinks")]).then(t=>{T=t[0].data,o=t[1].data,A(),H(),D(),v(o),U(o),k()}).catch(t=>{console.error("Error fetching data:",t)});
