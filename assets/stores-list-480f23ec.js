import"./bootstrap.min-feac71ab.js";import{d as A,a as M}from"./api-4c8ad0b6.js";import{A as P}from"./config-ca05e4f1.js";const g=document.querySelector("#storeList");let h=document.querySelector("#deliverySelect"),S=document.querySelector("#CooperationSelect"),x=[],d=[],v=[];const p=sessionStorage.getItem("user_id"),q=()=>{d.forEach(e=>{const t={uber:"uber eat",foodpanda:"food panda",hasEvent:"合作活動"};let a="";for(const s in t)e[s]&&(a+=`<li class="stores-tag">${t[s]}</li>`);v.push(a),a=""})},H=()=>{let e="";d.forEach(t=>{e+=`
        ...
        `}),g.innerHTML=e},j=()=>{H(),B(d)},y=()=>{const e=t=>document.querySelector(`.stores-card[data-shop-id="${t}"] .collect-btn`);x.forEach(t=>{const a=e(t.shopId);a&&(a.value="collected",a.classList.remove("fa-regular"),a.classList.add("fa-solid"))})};g.addEventListener("click",function(e){if(e.target.closest(".collect-btn")){if(!p)R();else if(e.target.classList.contains("collect-btn")){const a=e.target,l=a.closest(".stores-card").dataset.shopId,m=`${P}/userShopCollections?userId=${p}&shopId=${l}`;axios.get(m).then(f=>{if(f.data.length>0){A(p,l),a.value="uncollect",a.classList.remove("fa-solid"),a.classList.add("fa-regular"),Swal.fire("已取消收藏");return}else M(p,l),a.value="collected",a.classList.remove("fa-regular"),a.classList.add("fa-solid"),Swal.fire("收藏成功")}).catch(f=>{console.error("Error checking collection status:",f)})}}});function R(){Swal.fire("登入後即可使用收藏功能"),window.location.href="logIn.html"}h.addEventListener("change",$);S.addEventListener("change",$);function $(){let e=d;h.value!==""&&(h.value==="uber eat"?e=e.filter(t=>t.uber):h.value==="food panda"&&(e=e.filter(t=>t.foodpanda))),S.value!==""&&(e=e.filter(t=>t.hasEvent)),I(e),B(e),L(e),y()}function I(e){let t="";e.forEach(a=>{t+=`
    ...
    `}),g.innerHTML=t}const L=e=>{const t=e.length,a=12,s=Math.ceil(t/a);let l=1;function m(r){const n=(r-1)*a,o=n+a,b=e.slice(n,o);f(b),k(),y()}function f(r){g.innerHTML="";let n="";r.forEach(o=>{n+=`
          ...
          `}),g.innerHTML=n}function k(){const r=document.getElementById("pagination");r.innerHTML="";let n="";for(let i=1;i<=s;i++)n+=`
      ...
      </li>`,document.querySelectorAll(".page-link").forEach(i=>{i.addEventListener("click",E=>{E.preventDefault();const T=parseInt(i.dataset.page);u(T)})});const c=[l];document.getElementById("previousPageBtn").addEventListener("click",i=>{i.preventDefault(),c[0]>1&&u(c[0]-1)}),document.getElementById("next-page-btn").addEventListener("click",i=>{i.preventDefault(),c[0]!==s&&u(c[0]+1)}),document.getElementById("first-page-btn").addEventListener("click",i=>{i.preventDefault(),c[0]>1&&u(1)}),document.getElementById("last-page-btn").addEventListener("click",i=>{i.preventDefault(),c[0]!==s&&u(s)})}function u(r){l=r,m(l),window.scrollTo(0,400)}m(l)},B=e=>{const t=document.querySelector("#searchStores"),a=()=>{let s=e;t.value!==""&&(s=s.filter(l=>l.name.includes(t.value))),I(s),L(s),y()};t.addEventListener("keydown",s=>{s.keyCode===13&&a()})};Promise.all([axios.get(`${P}/userShopCollections?userId=${p}`),axios.get("https://json-server-project-wtkt.onrender.com/shops")]).then(e=>{x=e[0].data,console.log(x),d=e[1].data,q(),j(),$(),L(d),y()}).catch(e=>{console.error("Error fetching data:",e)});
